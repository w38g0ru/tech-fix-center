    ...
    function renderSectionHeader(string $sectionName, string $theme, bool $compact): string
    {
        ...
        return <<<HTML
                ...
                <div class="{$marginClass}">
                    <div class="px-4 mb-3">
                        <h3 class="text-xs font-medium {$textColor} uppercase tracking-wider">{$sectionName}</h3>
                    </div>
...
if (!function_exists('renderLightMenuItem')) {
    ...
    function renderLightMenuItem(array $item, bool $isActive, bool $compact): string
    {
        ...
        return <<<HTML
                <!-- {$name} -->
                <a href="{$url}"
                   class="nav-link flex items-center px-4 py-2 text-sm font-medium transition-colors duration-150 {$activeClass} mb-1"
                   title="{$name}">
                    <i class="{$icon} w-4 h-4 mr-3 flex-shrink-0 opacity-75"></i>
