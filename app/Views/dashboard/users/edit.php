...
<div class="w-full">
    <div class="bg-white shadow-xl rounded-2xl border border-gray-100">
        <form action="<?= base_url('dashboard/users/update/' . $user['id']) ?>" method="POST" class="p-6 lg:p-8 space-y-8">
            <?= csrf_field() ?>
            ...
            <div class="space-y-6">
                <div class="border-b border-gray-200 pb-4">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Update customer details and information</p>
                </div>
                ...
                <div>
                    ...
                    <option value="Registered" <?= old('user_type', $user['user_type']) === 'Registered' ? 'selected' : '' ?>>Registered</option>
                    <option value="Walk-in" <?= old('user_type', $user['user_type']) === 'Walk-in' ? 'selected' : '' ?>>Walk-in</option>
                </select>
            ...
            <div class="bg-gradient-to-r from-gray-50 to-gray-100 p-6 rounded-xl border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                    <div>
                        ...
                        <p><strong>सिर्जना:</strong> <?= formatNepaliDateTime($user['created_at'], 'medium') ?></p>
                    </div>
                    <div>
                        <p><strong>Current Type:</strong>
                            <span class="px-2 py-1 text-xs font-medium rounded-full <?= $user['user_type'] === 'Registered' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' ?>">
                                <?= esc($user['user_type']) ?>
                            </span>
                        </p>
                    </div>
            ...
            <div class="flex items-center justify-end gap-2 pt-8 border-t border-gray-200">
                ...
                <button type="submit"
                    ...
                    <span class="hidden md:inline md:ml-2 whitespace-nowrap">Update Customer</span>
                </button>
            </div>
        </form>
    </div>
